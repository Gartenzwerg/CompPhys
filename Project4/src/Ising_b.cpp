...
int main(int argc, char* argv[]) {
	...
	long cycleNum = 10000000*dim*dim;
...
void RandomSpinFlips(int** &Lattice, int dim, int cycleNum, double &energy, double J, int &MagMoment, double temp) {
	...
	susceptibility = (mean_m2 - mean_m_*mean_m_)/temp;

	// //per spin
	// mean_energy /= (dim*dim);
	// specific_heat /= (dim*dim*dim*dim);
	// mean_m /= (dim*dim);
	// susceptibility /= (dim*dim*dim*dim);

	cout<<" "<<temp<<"  "<<mean_energy<<"  "<<specific_heat<<"    "<<mean_m<<"    "<<susceptibility<<endl;
}